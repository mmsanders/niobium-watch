...
function signalRow(it) {
  ...
  return `<div class="signal"><span class="dot ${esc(itemRisk(it))}"></span><div><a href="${esc(it.url || "#")}" target="_blank" rel="noopener">${esc(itemTitle(it))}</a>${summary}</div><div class="muted" style="white-space:nowrap">${esc(itemDate(it))}</div></div>`;
}
...
function drawLine(monthly) {
  ...
  window._exportChart = new Chart(ctx, {
    ...
    options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { afterLabel: (c) => { const s = monthly[c.dataIndex]; return s ? "FOB " + fmtUsdM(s.fob_usd) + " · $" + (s.unit_value_usd_per_kg || "—") + "/kg" : ""; } } } },
      scales: { x: { ticks: { color: "#8b9aab", maxRotation: 45, autoSkip: true, maxTicksLimit: 16 }, grid: { color: "rgba(46,58,74,.5)" } }, y: { ticks: { color: "#8b9aab" }, grid: { color: "rgba(46,58,74,.5)" }, title: { display: true, text: "tonnes", color: "#8b9aab" } } } }
  });
}
