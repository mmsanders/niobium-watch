...
const TAB_STATUS = { niobium: null, helium: null };
...
function paintTabs() {
  document.querySelectorAll(".tab").forEach(b => {
    ...
    const status = TAB_STATUS[id] || "Watch";
    b.classList.remove("Normal", "Watch", "Stress");
    b.classList.add(status);
  });
}
async function refreshTabTemps(preferMeta, preferTab) {
  if (preferMeta && preferTab) {
    TAB_STATUS[preferTab] = preferMeta.overall_status || "Watch";
  }
  const missing = Object.keys(PATHS).filter(k => !TAB_STATUS[k]);
  await Promise.all(missing.map(async (k) => {
    try {
      const meta = await loadJson(PATHS[k].meta);
      TAB_STATUS[k] = meta.overall_status || "Watch";
    } catch (e) {
      TAB_STATUS[k] = TAB_STATUS[k] || "Watch";
    }
  }));
  paintTabs();
}
